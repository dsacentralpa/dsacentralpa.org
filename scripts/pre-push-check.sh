...
# Unpublished drafts must not reach production. DRAFT_MODE is a local review switch;
# committing it enabled would publish entries that have not been approved.
echo
echo "Draft gate"
if grep -qE '^\s*DRAFT_MODE\s*=\s*"true"' wrangler.toml 2>/dev/null; then
  fail "DRAFT_MODE is enabled in wrangler.toml"
  echo "          Unpublished issue and mutual-aid entries would go live on deploy."
  echo "          Set it back to \"false\" before committing."
else
  pass "DRAFT_MODE is off"
fi
# --------------------------------------------------------------- commit identity
echo
