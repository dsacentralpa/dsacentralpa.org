#!/usr/bin/env python3
"""
Central PA DSA — import the existing paper/spreadsheet list.

This REPLACES the old migrate.py, which pushed all 43 phone numbers straight into
Twilio. That would have been a problem: people who wrote their number on a clipboard
at a meeting have not given prior express written consent to receive automated texts,
and carriers can suspend a campaign over exactly that.

What this does instead:
  * imports EMAIL addresses as `pending`, and emails each person once asking them
    to confirm they still want to hear from us (re-permission);
  * imports phone numbers as reference data only, with phone_status='none'.
    Nobody is texted. Members join the SMS list themselves at dsacentralpa.org.

Three steps, run in order, each safe to inspect before the next:

  1. python scripts/import_list.py plan
       Reads the spreadsheet, cleans it, reports what it found.
       Writes  build/import.sql  and  build/repermission_queue.csv
       Sends nothing, touches no database.

  2. npx wrangler d1 execute dsa-list-db --remote --file=./build/import.sql
       Loads the rows into D1.

  3. python scripts/import_list.py send [--limit N] [--live]
       Sends the re-permission email. Without --live it prints a preview
       and sends nothing. Start with:  --limit 3 --live  to test on yourself.

Requires:  pip install pandas openpyxl requests python-dotenv
...
def cmd_plan(args) -> None:
    src = Path(args.file) if args.file else ROOT / "contact_list.xlsx"
    if not src.exists():
...
def main() -> None:
    ...
    p_plan.add_argument("--file", help="Path to the spreadsheet (default: contact_list.xlsx)")
